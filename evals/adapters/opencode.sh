#!/usr/bin/env bash
# Adapter: opencode CLI. Env: WORKDIR TASK_FILE TRANSCRIPT FINAL_FILE FINAL_MODE_FILE
...
case "${1:-run}" in
  ...
  run)
    ...
    [[ -z "${EVAL_EFFORT:-}" ]] || { echo "opencode adapter does not support --effort" >&2; exit 2; }
    args=(run)
    [[ -z "${EVAL_MODEL:-}" ]] || args+=(-m "$EVAL_MODEL")
    rc=0; opencode "${args[@]}" "$prompt" </dev/null > "$TRANSCRIPT" 2>&1 || rc=$?
    finalize || rc=${rc:-0}
  ...
  continue)
    ...
    exit $rc
    ;;
  *) echo "usage: opencode.sh install|run|continue|version|sandbox-mode" >&2; exit 1;;
esac
